...
- **Single request handler.** `src/server.ts` routes `/api/mcp` to the MCP transport; `/api/waitlist`, `/api/waitlist/config`, and `/api/waitlist/confirm` to the ownership flow; and everything else to static assets, with canonical-host 308 redirects for legacy domains (`src/http.ts`, `src/config.ts`).
...
- **Shared abuse limits and ownership proof.** The in-process limiter is only a cheap first pass. Firestore transactions enforce authoritative global cost/delivery ceilings plus independently scoped signed-client, submitted-address, and authenticated-confirmation-subject budgets across every Cloud Run instance; there is no low anonymous bucket one caller can fill to lock out every new browser (`src/waitlist-quota.ts`, `handleWaitlist()` and `handleWaitlistConfirm()` in `src/server.ts`). Join and confirmation each require a fresh, action-bound reCAPTCHA Enterprise score whose project, site key, exact hostname, age, validity, and minimum score are checked server-side (`src/recaptcha.ts`). A join remains pending until an explicit same-origin POST proves possession of the mailed, single-use Identity Platform code; scanner GETs only seal the code into a short-lived `Secure`, `HttpOnly`, `SameSite=Strict` cookie and redirect it out of browser history. Firestore compare-and-swap performs the final promotion (`src/identity-platform.ts`, `src/waitlist-confirmation.ts`, `src/waitlist.ts`).
- **No membership oracle.** New, pending, and confirmed addresses receive the same generic response and the same provider-send attempt after identical shared limits. The fixed Firestore transaction shape, bounded timing floor, and membership-independent mail dispatch keep storage, response, and provider behavior from disclosing whether an address was already present (`src/server.ts`, `src/waitlist.ts`).
...
- **Waitlist storage minimizes linkage.** Pending entries expire through an enforced Firestore TTL policy, quota documents have their own TTL policy, and confirmation uses a version-bound compare-and-swap. Entries store SHA-256 hashes of the opaque client cookie and user agent alongside the email; client IP addresses are neither trusted nor persisted (`src/waitlist.ts`, `infra/terraform/prod/main.tf`).
...
- `IDENTITY_PLATFORM_AUDIENCE`: exact project used for keyless OAuth ownership-mail dispatch and OOB-code checking
- `IDENTITY_PLATFORM_CONTINUE_URL`: exact canonical HTTPS callback; it must be `/api/waitlist/confirm` with no credentials, query, fragment, or alternate port
...
- `RECAPTCHA_PROJECT_ID`: exact project in which server-side Enterprise assessments are created
- `RECAPTCHA_SITE_KEY`: public score-key resource name injected directly from Terraform; the backend also verifies its exact hostname and action
- `WAITLIST_ACTIVATION_ENABLED`: exact `true` or `false`; defaults false and keeps join/config/confirmation fail closed until the protected infrastructure apply and live mailed round trip are proven
- `WAITLIST_BACKEND`: `file`, `memory`, or `firestore`; production uses `firestore`, pull request previews use `memory`
- `WAITLIST_IDENTITY_KEYSET`: one unpadded base64url-encoded 32-byte encryption key; deployed services require it. During local or private-deployment rotation, configure `new,old` so in-flight authenticated link and browser proofs remain readable through their short TTLs. Production never sources this value from GitHub. The trusted platform deploy reuses the one exact numeric Secret Manager version already bound to Cloud Run or, only when both the binding and enabled-version inventory are empty, generates a key and streams it directly into a new version before binding that numeric version. The platform generates a revision-local preview value during each trusted deploy.
